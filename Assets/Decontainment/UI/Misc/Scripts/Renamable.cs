...
public class Renamable : MonoBehaviour, IPointerClickHandler
{
    ...
    void Awake()
    {
        ...
        {
            if (s != prevText && (s == "" || (onRename != null && !onRename.Invoke(s)))) {
                if (s == "") {
                    PromptSystem.Instance.PromptInvalidAction("Invalid name!");
                } else {
                    PromptSystem.Instance.PromptInvalidAction(s + " already exists!");
                }
